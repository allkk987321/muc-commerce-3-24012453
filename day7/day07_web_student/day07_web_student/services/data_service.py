...
def load_dashboard_data(base_dir: Path, selected_category: str = "全部") -> dict:
    ...
    highest_churn = segment_df.loc[segment_df["流失率"].idxmax()]
    insight = (
        f"生命周期「{highest_churn['TenureGroup']}」阶段流失风险最高，"
        f"流失率为 {highest_churn['流失率']:.1%}（{int(highest_churn['流失人数'])}人/{int(highest_churn['用户数'])}人）。"
        f"随着使用时长增加，流失率逐步下降。"
    )
